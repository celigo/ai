...
The PRD's "Audit account health" skill composes four atomic reads — `list_resources` (`resourceType: "integrations"`), `list_resources` (`resourceType: "flows"`), `list_flow_errors` (no `_id`: one row per flow with its open-error count), `list_flow_runs` — and drills into the worst offenders. This prompt follows that pattern.

## Step 1: List integrations and flows

1. Call `list_resources` (`resourceType: "integrations"`) to enumerate all integration containers. Note name, mode, and how many flows each one has.
2. Call `list_resources` (`resourceType: "flows"`) to enumerate every flow. Capture:
...
Call `list_flow_errors` without `_id` (optionally scoped with `_integrationId`) — it returns one row per flow with `numOpenError`, sorted worst first, in a single call; `numError_gte: 0` includes flows with zero errors. Skip disabled flows; they don't generate new errors.
...
For the top 3-5 flows with the highest error counts, call `list_flow_runs` (`_flowId`, newest first) to get the latest job. Examine:
...
If the user wants a wider time window, call `list_flow_runs` with a `_flowId` filter and `createdAt_gt` set to ~30 days back. This shows whether errors are recent (recent regression) or longstanding (chronic issue).
...
1. `get_resource` (`resourceType: "flows"`) to understand the flow's structure (source, destination, schedule, scripts).
2. `list_flow_errors` with the flow `_id` (then `_id` + `_stepId`) to read actual error messages; add `status: "resolved"` to compare with errors that already cleared.
...
Call `list_resources` (`resourceType: "connections"`). For each connection look for:
...
Use the `list_resources` (`resourceType: "integrations"`) data already collected to spot:
